...
def get_req(url, attempts=0):
    if attempts == 3:
        print_error(f'Request for "{url}" failed three times in a row, skipping...')
        return ''

    try:
        r = sess.get(url)
    except requests.exceptions.RequestException:
        print_warning(f'Request for "{url}" failed, retrying in 5 secs...')
        sleep(5)
...
def scrape(parent_url):
    ...
    if verbosity > 1:
        print_info(f'Scraper: scraping links from "{parent_url}"')
...
def scraper(nthreads):
    ...
    print_status(f'Scraping Moodle on "{base_url}"')
...
def scrape_moodle(args, session):
    ...
    print_success(f'Saved scraping result to file "{jsonfile}"')
